...
function WinLossBar({ wins, losses }: { wins: number; losses: number }) {
  ...
  return (
    <View style={wlStyles.container}>
      <View style={[wlStyles.winSeg, { flex: wins }]}>
        <Text style={wlStyles.winLabel}>{wins}W</Text>
      </View>
      <View style={[wlStyles.lossSeg, { flex: losses }]}>
    ...
    <View style={fcStyles.container}>
      ...
      <View style={{ flex: 1 }}>
        <View style={fcStyles.topRow}>
          ...
          <View style={[
            fcStyles.recordBadge,
            { backgroundColor: isWinning ? Colors.tealDim : Colors.redDim }
          ]}>
...
function PlayerHeader() {
  const { playerName, playerStreak, playerTotalKm, playerZones, playerColorIndex } = useGame();
  const zc = ZoneColors[playerColorIndex];

  return (
    <LinearGradient
      colors={[Colors.bg3, Colors.bg2]}
      style={phStyles.container}
    >
      <View style={phStyles.avatarWrap}>
        <LinearGradient
          colors={[zc.stroke + "40", zc.stroke + "10"]}
      ...
      <View style={phStyles.info}>
        <Text style={phStyles.name}>{playerName}</Text>
        <View style={phStyles.badgeRow}>
          <View style={[phStyles.badge, { backgroundColor: Colors.tealDim }]}>
            ...
            <Text style={[phStyles.badgeText, { color: Colors.teal }]}>Mumbai</Text>
          </View>
...
const phStyles = StyleSheet.create({
  ...
  name: { fontFamily: "Inter_700Bold", fontSize: 22, color: Colors.text, marginBottom: 8 },
          ...
          <View style={styles.tiersCard}>
            {[
              ...
              <View key={tier.label} style={styles.tierRow}>
                ...
                <View style={[styles.tierColorLine, { backgroundColor: tier.color + "30" }]}>
                  <Text style={[styles.tierLabel, { color: tier.color }]}>{tier.label}</Text>
                </View>
                <Text style={styles.tierRange}>{tier.range}</Text>
                <Text style={[styles.tierDecay, { color: tier.decay.startsWith("−0") ? Colors.teal : Colors.text2 }]}>
                  {tier.decay}
                </Text>
              </View>
            ))}
